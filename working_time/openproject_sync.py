...
def _ensure_project(project_id: str, project_payload: dict[str, Any] | None = None) -> str:
	site_name = _single_site_name()
	...
	project_doc = _new_project_doc(project_id, identifier, notes, customer_name)
...
def _time_entry_row_values(site_name: str, time_entry: dict[str, Any]) -> dict[str, Any]:
	...
	project_name = _ensure_project(project_id, project_payload or None)
	work_package_id = (
...
def _upsert_time_entry(time_entry: dict[str, Any]) -> str:
	site_name = _single_site_name()
	client = OpenProjectClient(site_name)
...
def sync_work_package_from_openproject(work_package_id: str, site_name: str | None = None) -> dict[str, Any]:
	...
	project_name = _ensure_project(project_id)
	return {"created_or_updated": True, "project": project_name}
...
def sync_time_entry_from_openproject(time_entry_id: str, site_name: str | None = None) -> dict[str, Any]:
	...
	action = _upsert_time_entry(time_entry)
	return {action: True}


@frappe.whitelist()
def poll_openproject_time_entries(openproject_site: str | None = None) -> dict[str, Any]:
	site_name = _single_site_name()
	client = OpenProjectClient(site_name)
	...
	for time_entry in _iterate(
		client,
		"/time_entries",
		params={"sortBy": json.dumps([["updatedAt", "asc"], ["id", "asc"]])},
	):
		action = _upsert_time_entry(time_entry)
